...
def take_survey(id):
    ...
    # Проверка по IP (упрощённо)
        has_answered = db_session.query(Answer).filter(Answer.ip_address == request.remote_addr).join(Question).filter(Question.survey_id == id).first()
            ...
            for question in survey.questions:
                ...
                if question.type == QuestionType.FILE:
                    ...
                    if question.type in [
                        ...
                        values = request.form.getlist(f'q_{question.id}') # request.form.get(f'q_{question.id}')
                        print("values!", values)
                        for value in values:
                            ...
                            value = int(value)
                            try:
                                _answer.text_response = db_session.query(Option).filter(Option.id == value, Option.question_id == question.id).first().text
                            except Exception as e:
